          ...
          <div className="grid gap-10 lg:grid-cols-2 lg:gap-16">
            ...
            <div className="flex flex-col">
              ...
              <div className="mt-6 flex items-center gap-4">
                <div className="flex items-center gap-2 rounded-md border border-border-primary bg-bg-card px-4 py-2.5">
              ...
              <div className="mt-8">
                ...
                <div className="grid grid-cols-2 gap-px overflow-hidden rounded-md border border-border-primary bg-border-primary">
                  {Object.entries(product.specs).map(([key, value]) => (
                    <div
                      key={key}
                      className="flex flex-col bg-bg-card px-4 py-3"
                    >
              ...
              {product.tags.length > 0 && (
                <div className="mt-6 flex flex-wrap gap-2">
                  {product.tags.map((tag) => (
                    <span
                      key={tag}
                      className="rounded bg-bg-elevated px-2.5 py-1 text-[10px] font-medium uppercase tracking-wider text-text-muted"
                    >
                      {tag}
                    </span>
                  ))}
              ...
              <div className="mt-8">
                ...
                <p className="mt-3 text-xs text-text-muted">
                  {session.isAuthenticated
                    ? "You will be directed to our quote request system."
                    : "Sign in or register to request a quote on this product."}
                </p>
              </div>
            </div>
          </div>
